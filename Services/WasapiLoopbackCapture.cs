...
/// 纯 P/Invoke（无第三方依赖），失败时以 IsRunning=false / Error 返回，由频谱引擎回退模拟。
...
public sealed class WasapiLoopbackCapture : IDisposable
{
    ...
    private bool TryInit()
    {
        try
        {
            ...
            var hr = enumerator.GetDefaultAudioEndpoint(EDataFlowRender, ERoleConsole, out var device);
            if (hr < 0 || device == null)
                throw new InvalidOperationException($"GetDefaultAudioEndpoint hr=0x{hr:X8}");

            var iidClient = IID_IAudioClient;
            hr = device.Activate(ref iidClient, CLSCTX_ALL, IntPtr.Zero, out var ac);
            if (hr < 0 || ac == null)
                throw new InvalidOperationException($"Activate(IAudioClient) hr=0x{hr:X8}");

            _client = (IAudioClient)ac;
            ...
            // 仅支持 float32 / 16bit PCM，其余格式回退模拟
            if (_floatFormat && fmt.wBitsPerSample != 32)
                throw new NotSupportedException($"unsupported float format {fmt.wBitsPerSample}bit");
            if (!_floatFormat && fmt.wBitsPerSample != 16)
                throw new NotSupportedException($"unsupported pcm format {fmt.wBitsPerSample}bit");
    ...
    private void CaptureLoop()
    {
        try
        {
            while (_running && _wait != null)
            {
                if (!_wait.WaitOne(1000))
                    continue;
                Drain();
            }
        }
    ...
    {
        [PreserveSig]
        int Activate(ref Guid iid, int clsCtx, IntPtr activationParams, out object ppInterface);
    ...
    private interface IAudioClient
    {
        ...
        [PreserveSig]
        int GetService(ref Guid iid, out object ppInterface);
    }
